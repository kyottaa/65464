...
// Фильтрация списка песен по запросу
searchInput.addEventListener('input', function() {
    const searchValue = this.value.toLowerCase();
    clearSearchButton.style.display = searchValue ? 'inline' : 'none'; // Показать кнопку очистки, если есть текст

    const songItems = document.querySelectorAll('#song-list li');
    songItems.forEach(item => {
        const songTitle = item.querySelector('span').textContent.toLowerCase();
        const artistName = item.querySelector('.artist-name').textContent.toLowerCase();

        // Скрыть элементы, не соответствующие запросу
...
clearSearchButton.addEventListener('click', function() {
    ...
    searchInput.dispatchEvent(new Event('input')); // Обновить список песен
    searchInput.blur(); // Скрыть клавиатуру
    clearSearchButton.style.display = 'none'; // Скрыть кнопку очистки
});
